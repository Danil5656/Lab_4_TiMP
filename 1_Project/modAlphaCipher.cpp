...
wstring modAlphaCipher::getValidKey(const wstring & s)
{
    ...
    for (int i = 0; i < wst.size(); i++) {
        if (iswalpha(wst[i])==0) {
            throw Error(string("в вашем ключе присутствуют недопустимые символы."));
            break;
        }
        if (iswlower(wst[i]))
...
wstring modAlphaCipher::getValid_Str(const wstring & s)
{
    ...
    locale::global(loc);
    if (s.empty())
        throw Error(string("вы ввели пустой текст."));
    wstring wst = s;
    for (int i = 0; i < wst.size(); i++) {
        if (iswalpha(wst[i])==0) {
            throw Error(string("в вашем тексте присутствуют недопустимые символы."));
            break;
        }
        if (iswlower(wst[i]))
            wst[i] = towupper(wst[i]);
